...
- [Proxying API Requests in Development](#proxying-api-requests-in-development)
  - ["Invalid Host Header" Errors After Configuring Proxy](#invalid-host-header-errors-after-configuring-setupProxy)
  - [Configuring the Proxy Manually](#configuring-the-setupProxy-manually)
...
- `src/OverheadModalTable.js` is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/OverheadModalTable.js`, but you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your `src/OverheadModalTable.js` file:
...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/OverheadModalTable.js` file, for example:
...
To tell the development server to setupProxy any unknown requests to your API server in development, add a `setupProxy` field to your `package.json`, for example:

```js
  "setupProxy": "http://localhost:4000",
```

This way, when you `fetch('/api/todos')` in development, the development server will recognize that it’s not a static asset, and will setupProxy your request to `http://localhost:4000/api/todos` as a fallback. The development server will **only** attempt to send requests without `text/html` in its `Accept` header to the setupProxy.
...
Keep in mind that `setupProxy` only has effect in development (with `npm start`), and it is up to you to ensure that URLs like `/api/todos` point to the right thing in production. You don’t have to use the `/api` prefix. Any unrecognized request without a `text/html` accept header will be redirected to the specified `setupProxy`.

The `setupProxy` option supports HTTP, HTTPS and WebSocket connections.<br>
If the `setupProxy` option is **not** flexible enough for you, alternatively you can:

- [Configure the setupProxy yourself](#configuring-the-setupProxy-manually)
- Enable CORS on your server ([here’s how to do it for Express](http://enable-cors.org/server_expressjs.html)).
- Use [environment variables](#adding-custom-environment-variables) to inject the right server host and port into your app.

### "Invalid Host Header" Errors After Configuring Proxy

When you enable the `setupProxy` option, you opt into a more strict set of host checks. This is necessary because leaving the backend open to remote hosts makes your computer vulnerable to DNS rebinding attacks. The issue is explained in [this article](https://medium.com/webpack/webpack-dev-server-middleware-security-issues-1489d950874a) and [this issue](https://github.com/webpack/webpack-dev-server/issues/887).

This shouldn’t affect you when developing on `localhost`, but if you develop remotely like [described here](https://github.com/facebook/create-react-app/issues/2271), you will see this error in the browser after enabling the `setupProxy` option:
...
If the `setupProxy` option is **not** flexible enough for you, you can get direct access to the Express app instance and hook up your own setupProxy middleware.

You can use this feature in conjunction with the `setupProxy` property in `package.json`, but it is recommended you consolidate all of your logic into `src/setupProxy.jsxy.js`.

First, install `http-setupProxy-middleware` using npm or Yarn:

```bash
$ npm install http-setupProxy-middleware --save
$ # or
$ yarn add http-setupProxy-middleware
```

Next, create `src/setupProxyProxy.js` and place the following contents in it:

```js
const setupProxy = require('http-setupProxy-middleware');
...
You can now register proxies as you wish! Here's an example using the above `http-setupProxy-middleware`:

```js
const setupProxy = require('http-setupProxy-middleware');

module.exports = function(app) {
  app.use(setupProxy('/api', { target: 'http://localhost:5000/' }));
};
```
...
You may require the dev server to serve pages over HTTPS. One particular case where this could be useful is when using [the "setupProxy" feature](#proxying-api-requests-in-development) to setupProxy requests to an API server when that API server is itself serving HTTPS.
...
following in their [`src/OverheadModalTable.js`](src/index.js) file:
...
- If the watcher doesn’t see a file called `OverheadModalTable.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
